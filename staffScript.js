...
//When staff clicks on "Add/Remove Guest" button on details page
app.post("/modifyGuest.html", function(req, response){
	...
	MongoClient.connect(dbURL, function(err1, db){
		...
		dbo.collection("reservation").findOne({confirmationNumber: confirmationNumber}, function(err2, reservation){
			if (err2)
				throw err2;
			response.render("staffModifyGuestEJS", {reservation: reservation, userID: userID});
		})
	})
})

//When staff clicks on "Add Guest" button on modify guest page
//Ask staff how many additional guests to add
...
app.post("/addGuest.html", function(req, response){
	...
	MongoClient.connect(dbURL, function(err1, db){
		...
		dbo.collection("reservation").findOne({confirmationNumber: confirmationNumber}, function(err2, reservation){
			if (err2)
				throw err2;
			response.render("staffAddGuestEJS", {reservation: reservation, userID: userID});
		})
	})	
})


//When staff clicks on "Add Guest" button after entering in adult/children amounts to add
		...
		dbo.collection("reservation").findOne({confirmationNumber: confirmationNumber}, function(err2, reservation){
			...
			dbo.collection("room").find({}).toArray(function(err3, rooms){
				...
				if (reservation.adults + reservation.children + adults + children > occupancy){
					alert("The number of guests in the reservation will exceed the occupancy of its booked rooms. Please add additional rooms.");
					return;
				}
				else{
...
//When staff clicks on "Remove Guest" button on modify guest page
//Allow staff to remove all guests, down to one adult
app.post("/removeGuest.html", function(req, response){
	...
	MongoClient.connect(dbURL, function(err1, db){
		...
		dbo.collection("reservation").findOne({confirmationNumber: confirmationNumber}, function(err2, reservation){
			if (err2)
				throw err2;
			response.render("staffRemoveGuestEJS", {reservation: reservation, userID: userID});
		})
	})		
})

//When staff clicks on "Remove Guest" button after entering in a number of adults and children to remove
	...
	MongoClient.connect(dbURL, function(err1, db){
		...
		dbo.collection("reservation").findOne({confirmationNumber: confirmationNumber}, function(err2, reservation){
			...
			if (reservation.adults - adults < 1){
				alert("You cannot remove this many adults from the reservation.");
				return;
			}
			else{
